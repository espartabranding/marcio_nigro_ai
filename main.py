...
async def parse_with_llama(content: bytes, filename: str) -> str:
    async with httpx.AsyncClient(timeout=300) as client:
        upload = await client.post(
            ...
            data={
                "language": "pt",
                "parsing_instruction": "Extraia todo o conteudo detalhadamente. Para graficos e tabelas, descreva os dados numericamente. Para slides, extraia titulo, bullets e dados visuais.",
            }
        )
        ...
        if not job_id:
            raise HTTPException(status_code=502, detail="LlamaParse nao retornou job_id")
        for _ in range(60):
            ...
            if job_status == "SUCCESS":
                ...
                raise HTTPException(status_code=502, detail="LlamaParse job falhou")
...
async def extract_text(file: UploadFile) -> str:
    ...
    if name.endswith(".docx"):
        try:
            ...
            raise HTTPException(status_code=422, detail="python-docx nao instalado")
    raise HTTPException(status_code=415, detail=f"Formato nao suportado: {name}")
...
async def generate_answer(query: str, matches: list, client_name: str) -> str:
    parts = []
    for m in matches:
        source = m.get("metadata", {}).get("source", "desconhecido")
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Seja direto, claro e estruturado. Use listas numeradas quando fizer sentido."
            )
        },
        {
            "role": "user",
    ...
    if r.status_code != 200:
        raise HTTPException(status_code=502, detail=f"GPT error: {r.text}")
...
@app.get("/health")
async def health():
    return {"status": "ok", "version": "2.0.0", "llama_parse": bool(LLAMA_API_KEY)}
...
MCP_TOOLS = [
    {"name": "search_knowledge_base", "description": "Busca semantica na base de conhecimento.",
     "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "top_k": {"type": "integer", "default": 5}}, "required": ["query"]}},
    {"name": "ask_knowledge_base", "description": "Pergunta com resposta gerada por IA baseada na base de conhecimento.",
     "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "top_k": {"type": "integer", "default": 5}}, "required": ["query"]}},
    {"name": "ingest_text", "description": "Ingere texto na base de conhecimento.",
     "inputSchema": {"type": "object", "properties": {"text": {"type": "string"}, "source_name": {"type": "string"}}, "required": ["text"]}},
    {"name": "get_index_stats", "description": "Estatisticas do indice Pinecone.",
...
async def mcp_call(request: Request, client=Depends(get_client)):
    ...
    if name == "search_knowledge_base":
        result = await search(QueryRequest(query=inp["query"], top_k=inp.get("top_k", 5), namespace_override=namespace), client)
        return {"type": "tool_result", "content": result}

    elif name == "ask_knowledge_base":
        result = await ask(QueryRequest(query=inp["query"], top_k=inp.get("top_k", 5), namespace_override=namespace), client)
        return {"type": "tool_result", "content": result}
        ...
        vectors = [
            {"id": f"{doc_id}_chunk_{i}_{uuid.uuid4().hex[:4]}", "values": emb,
    ...
    raise HTTPException(status_code=404, detail=f"Tool '{name}' nao encontrada")
